...
public class Protocol {
    ...
    //упаковка имени пользователя в json
    ...
    //узнать тип входящего сообщения
    ...
    //распаковать статус пользователя
    public static UserStatus unpackStatus(String json){
        Gson g = new Gson();
        return  g.fromJson(json.substring(1), UserStatus.class);
    }

    //распаковать имя пользователя
    public static String unpackUsername(String json){
        Gson g = new Gson();
        return  g.fromJson(json.substring(1), UserStatus.class).getUser().getName();
    }
}
